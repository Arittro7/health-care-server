...
const createAppointment = async (
  ...
  const result = await prisma.$transaction(async (tnx) => {
    ...
    const paymentData = await tnx.payment.create({ //variable add
      data: {
    ...
    // chat gpt generated 👆🏻 Provide the prompt for this exact code generate 
    // console.log(session);
    return {paymentUrl: session.url}
    // webhook will be called after the payment
  });
...
const cancelUnpaidAppointments = async () => {
    ...
    await prisma.$transaction(async (tnx) => {
        await tnx.payment.deleteMany({
            where: {
                appointmentId: { //Payment Schema has appointmentId field 
